...
class _ProfilePageState extends State<ProfilePage>
  ...
  void initState() {
    ...
    // Use post-frame callback to ensure Provider is accessible
  ...
  void _loadUserStats() async {
    ...
    try {
      ...
      // Update states in a safe way
      if (mounted) {
        setState(() {
          _roomsJoined =
              roomProvider.activeRooms.length +
              roomProvider.pendingRooms.length;
  ...
  // Method to safely refresh rooms data
  void _refreshRooms() {
    try {
      if (!mounted) return;

      // Use a post-frame callback to ensure the context is attached to the widget tree
      WidgetsBinding.instance.addPostFrameCallback((_) {
        try {
          ...
          print('Error refreshing rooms in post-frame callback: $e');
        }
      });
    } catch (e) {
