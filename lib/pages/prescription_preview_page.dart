...
import 'save_prescription_dialog.dart';
...
class _PrescriptionPreviewPageState extends State<PrescriptionPreviewPage> {
  ...
  void initState() {
    ...
    // If this is from a saved prescription, set the audio path
    if (widget.savedPrescription != null &&
        widget.savedPrescription!.hasAudio) {
      _audioPath = widget.savedPrescription!.audioUrl;
      _hasRecording = true;
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        elevation: 0,
        actions: [
          // Save prescription button (only show if not from saved prescription)
          if (widget.savedPrescription == null)
            IconButton(
              onPressed: _isUploading ? null : _showSaveDialog,
  ...
  void _showSaveDialog() async {
    ...
    if (user == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Please log in to save prescriptions'),
          backgroundColor: Colors.red,
        ),
      );
      return;
    ...
    if (imageUrls.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Error uploading images for saving'),
          backgroundColor: Colors.red,
        ),
      );
      return;
    ...
    final result = await showDialog<bool>(
      context: context,
      builder:
          (context) => SavePrescriptionDialog(
            imageUrls: imageUrls,
            audioUrl: _audioPath,
          ),
    );
  ...
  Future<List<String>> _uploadImages() async {
    ...
    for (int i = 0; i < widget.imagePaths.length; i++) {
      try {
        ...
        final fileName =
            'prescription_${DateTime.now().millisecondsSinceEpoch}_$i.jpg';
        final ref = FirebaseStorage.instance.ref().child(
          'prescriptions/${widget.roomId}/$fileName',
        );
  ...
  Future<void> _submitPrescription() async {
    ...
    try {
      ...
      // If from saved prescription, use existing URLs
      if (widget.savedPrescription != null) {
        uploadedImageUrls = List.from(widget.savedPrescription!.imageUrls);
        uploadedAudioUrl = widget.savedPrescription!.audioUrl;
        setState(() {
          _uploadProgress = 0.8; // Skip image upload
        });
      } else {
      ...
      // Upload audio if exists (only if not from saved prescription)
      if (widget.savedPrescription == null &&
          _hasRecording &&
          _audioPath != null) {
        setState(() {
